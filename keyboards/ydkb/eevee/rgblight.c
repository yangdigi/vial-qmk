...
rgblight_config_t rgblight_config = {.enable = 0, .mode = 8,.hue = 192, .sat = 255, .val = 255};
...
{
    memset(&rgbled[1], 0, sizeof(rgbled)-3);
}

#define RGB_FADING_STEP 2
void rgblight_set(void)
{
    if (display_connection_status_check_times) return;
...
void rgblight_effect_rainbow_swirl(uint8_t interval)
{
    ...
    for (i=0; i<RGBLED_NUM; i++) {
        hue = 256/16*i+current_hue;
        sethsv(hue, rgblight_config.sat, rgblight_config.val, &led[i]);
    }
    rgblight_set();
    current_hue = current_hue - interval2*4;
}
...
void hook_keyboard_loop()
{
    ...
    if (timer_elapsed(rgb_update_timer) > 40) {
        ...
        // led_status_task();
        rgbled[0] = (struct cRGB){ .r = 0, .g = 0, .b = 0 }; 
