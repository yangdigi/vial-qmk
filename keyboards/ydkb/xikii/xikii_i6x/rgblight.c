...
#define RGBLED_TEMP  (RGBLED_NUM*2 + 1) //29
...
struct cRGB led[RGBLED_NUM*2+2]; //29+1
...
void rgblight_set(void)
{
    ...
    //reduce the brightness of ws2812 in the front (RGB16 to RGB29)
    led[14] = led[13];
    for (uint8_t i=15;i<29;i++) {
        led[i].r = led[28-i].r/4;
        led[i].g = led[28-i].g/4;
        led[i].b = led[28-i].b/4;
    }
    ws2812_setleds(led);
}
...
void hook_keyboard_loop()
{
    ...
    if (rgblight_timer_enabled && timer_elapsed(rgb_update_timer) > 30) {
