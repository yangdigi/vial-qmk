...
#define rgblight_timer_enable() do { PORTD &= ~(1<<4);} while(0)
#define rgblight_timer_disable() do { PORTD |= (1<<4);} while(0)
...
void rgblight_set(void)
{
    ...
    //    rgblight_clear();
    //} 
    //reduce the brightness of ws2812
    // 淡入淡出，灯少所以不用降低亮度。连接指示时不用降低亮度
    struct cRGB connection_color = led[0]; //save connection led color
...
void suspend_wakeup_init_action(void)
{
    ...
    if (BT_POWERED == 0) display_connection_status_check_times = 1;
}
    ...
    if (timer_elapsed(battery_timer) > 150) {
        ...
        if (low_battery) {
            if (ble51_task_steps > 3) {
                ...
                if (USB_DeviceState != DEVICE_STATE_Configured) { //not charging
                    backlight_disable();
                    rgblight_timer_disable(); 
                    PORTB ^= (1<<7);
                } else {
